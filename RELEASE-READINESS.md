...
| D6 | **Settings "About" section** — no version display, support contact, or discoverable Restore Purchases | 🟢 | June 10 | **DONE.** Added an always-visible About section to `SettingsView`: version/build row (`CFBundleShortVersionString` (`CFBundleVersion`)), **Restore Purchases** button → new `SubscriptionManager.restorePurchases()` (mirrors `refreshStatus()`, reuses `updateProStatus`) with a result alert, and an iOS-only Contact Support `mailto` (`support@komodollc.com`). Restore also still lives in the RevenueCat paywall footer (3.1.2 already satisfied). iOS build clean. |
| D7 | **Force-unwrap hygiene** — guarded `!` not covered by B1 | 🟢 | June 10 | **DONE.** De-`!`'d 8 `Calendar.date(byAdding:)` / `DateFormatter.date(from:)` calls in `Date+Extensions.swift` (`?? Date()`/`?? self`); made cache-dir access empty-array-safe in `Cache.saveToDisk` (also fixed latent `URL(string: absoluteString + name)!` bug → `appendingPathComponent`) and `GameViewModel` cache load (`folderURLs.first` guarded inside the existing `do/catch`). Behavior-preserving. `NetworkHandler:479 realLiveScore!` verified safe — left as-is. |
