        ...
        VStack(spacing: 4) {
            if let game = entry.game?.first {
                if let homeTeamID = game.idHomeTeam, let homeTeam = Team.getTeamInfoFrom(teams: entry.teams, teamID: homeTeamID), let awayTeamID = game.idAwayTeam, let awayTeam = Team.getTeamInfoFrom(teams: entry.teams, teamID: awayTeamID) {
                    RectangularWidgetTeamView(shortName: awayTeam.strTeamShort, longName: awayTeam.strTeam , isAway: true, data: entry.images?[awayTeamID])
                    RectangularWidgetTeamView(shortName: homeTeam.strTeamShort, longName: homeTeam.strTeam, isAway: false, data: entry.images?[homeTeamID])
