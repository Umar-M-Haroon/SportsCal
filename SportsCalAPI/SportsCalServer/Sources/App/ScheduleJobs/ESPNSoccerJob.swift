...
struct ESPNSoccerJob: AsyncScheduledJob {
    ...
    func run(context: Queues.QueueContext) async throws {
        let endpoint = RedisEndpoint.ESPN.allSoccerScoreboards.getValue(isDebug: context.application.environment == .development)
        let soccerScoreboards = try await context.application.redis.get(endpoint, asJSON: [Leagues: Scoreboard].self)
        ...
        if leaguesToFetch == Leagues.allCases.filter({$0.isSoccer}) {
            try await context.application.redis.setex(RedisEndpoint.ESPN.allSoccerScoreboards.getValue(isDebug: context.application.environment == .development), toJSON: espnInfo, expirationInSeconds: 60 * 60 )
        }
